"""Optional retrofit model training.

The default recommendation approach is similarity-voting; this model is optional.
"""
...
def _make_estimator(cfg: ProjectConfig, name: str):
    ...
    raise ValueError(f"Unknown retrofit_model: {name}")


def train_retrofit_model(
    cfg: ProjectConfig,
    df: pd.DataFrame,
    preprocessors: Preprocessors,
    feature_cols: list[str],
) -> tuple[Any, dict[str, Any]]:
    if cfg.training.retrofit_model.lower() in {"off", "none"}:
        raise RuntimeError("Retrofit model training is disabled (set training.retrofit_model in config)")

    target = cfg.columns.retrofit_target
    if target not in df.columns:
        raise KeyError(f"Retrofit target column not found: {target}")
    ...
    est = _make_estimator(cfg, cfg.training.retrofit_model)
